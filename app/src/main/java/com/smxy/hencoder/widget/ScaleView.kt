...
class ScaleView : View {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        //长度
        mArcLength = (Math.PI * 2 * ((mRect.width() / 2) - DASH_WIDTH / 2) * SWEEP_ANGLE / 360F).toFloat()
        dashPathEffect = DashPathEffect(
            floatArrayOf(
                SCALE_WIDTH.toFloat(),
                ((mArcLength - (MAX_SCALE + 1) * SCALE_WIDTH.toFloat()) / MAX_SCALE).toFloat()
            ), 0F
        )
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawArc(
            mRect.left + DASH_WIDTH / 2,
            mRect.top + DASH_WIDTH / 2,
            mRect.right - DASH_WIDTH / 2,
            mRect.bottom - DASH_WIDTH / 2,
            START_ANGLE,
            SWEEP_ANGLE,
            false,
            mPaint
